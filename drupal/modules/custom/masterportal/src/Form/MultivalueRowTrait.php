...
trait MultivalueRowTrait {
  ...
  final protected function createMultivalueFormPortion(array &$container, $property, FormStateInterface $form_state, array $default_values = [], $empty_text = FALSE) {
    ...
    if (!empty($rows)) {
      foreach ($rows as $delta => $row_values) {
        ...
        if ($this->canRowsBeRemoved($property)) {
        ...
        $row_inputs = $this->getInputRow($property, $delta, $row_values, $form_state);
    ...
    if ($this->canRowsBeAdded($property)) {
   ...
   */
  final protected static function findData($property, array $userInput) {
    foreach ($userInput as $key => $value) {
      if ($key === $property && isset($value['multivaluePart'])) {
        return $value['multivaluePart'];
      }
      elseif (is_array($value) && ($subvalue = self::findData($property, $value)) !== FALSE) {
        return $subvalue;
      }
    }
    return FALSE;
   ...
   */
  final protected static function getData($property, array $userInput) {
    $raw = self::findData($property, $userInput);
    $transformed = [];
    if (isset($raw['rows']) && is_array($raw['rows'])) {
      foreach ($raw['rows'] as $current) {
        $rowdata = [];
        foreach ($current['row'] as $key => $data) {
          if (in_array($key, ['delta', 'remove'])) {
            continue;
          }
  ...
  final protected function getAddSelection($property, array $userInput) {
    $rawData = self::findData($property, $userInput);
  ...
  final public function addRow(array &$form, FormStateInterface $form_state) {
    ...
    $values = self::getData($property, $user_input);
  ...
  /**
   * Determines if rows can be removed.
   *
   * @param string $property
   *   The name of the property.
   *
  ...
  /**
   * Determines if new rows can be added.
   *
   * @param string $property
   *   The name of the property.
   *
   ...
   *
   * @param string $property
   *   The property this input row is for.
   * @param int $delta
   ...
   */
  abstract protected function getInputRow($property, $delta, array $row_defaults, FormStateInterface $form_state);
